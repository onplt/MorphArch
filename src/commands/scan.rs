...
fn is_test_path(path: &Path) -> bool {
    ...
    const TEST_DIRS: &[&str] = &[
        "/test/", "/tests/", "/testdata/", "/test_data/", "/__tests__/", "/spec/", "/fixtures/", "/fixture/",
        "/examples/", "/example/", "/benchmarks/", "/bench/", "/testutil/", "/testing/", "/mock/", "/mocks/",
        "/snapshots/", "/e2e/",
    ];
    for pat in TEST_DIRS { if lower.contains(pat) { return true; } }
    const TEST_DIR_PREFIXES: &[&str] = &[
        "test/", "tests/", "testdata/", "test_data/", "__tests__/", "spec/", "fixtures/", "fixture/",
        "examples/", "example/", "benchmarks/", "bench/", ".github/",
    ];
    for pat in TEST_DIR_PREFIXES { if lower.starts_with(pat) { return true; } }
    let file_name = path.file_name().unwrap_or_default().to_string_lossy().to_ascii_lowercase();
    file_name.ends_with("_test.ts") || file_name.ends_with("_test.tsx") || file_name.ends_with("_test.rs") ||
    file_name.ends_with("_test.go") || file_name.ends_with("_test.py") || file_name.ends_with(".test.ts") ||
    file_name.ends_with(".test.tsx") || file_name.ends_with(".test.js") || file_name.ends_with(".spec.ts") ||
    file_name.ends_with(".spec.tsx") || file_name.starts_with("test_")
}

fn is_noise_import(name: &str) -> bool {
    let name = name.trim();
    if name.len() <= 1 { return true; }
    if name.starts_with("http://") || name.starts_with("https://") { return true; }
    if name.starts_with("npm:") || name.starts_with("node:") { return true; }
    let lower = name.to_ascii_lowercase();
    if lower.ends_with(".css") || lower.ends_with(".scss") || lower.ends_with(".json") || lower.ends_with(".svg") ||
       lower.ends_with(".png") || lower.ends_with(".jpg") || lower.ends_with(".wasm") || lower.ends_with(".html") ||
       lower.ends_with(".md") || lower.ends_with(".txt") { return true; }
    if name.starts_with(|c: char| c.is_ascii_digit()) && name.contains('.') { return true; }
...
fn normalize_import(name: &str) -> String {
    ...
    if let Some(rest) = name.strip_prefix("npm:") {
        let without_version = if let Some(stripped) = rest.strip_prefix('@') {
            match stripped.find('@') { Some(pos) => &rest[..pos + 1], None => rest }
        } else { rest.split('@').next().unwrap_or(rest) };
        return without_version.to_string();
    }
    if let Some(rest) = name.strip_prefix("node:") { return rest.to_string(); }
    name.to_string()
}

fn collect_edges(source_pkg: &str, imports: &[String], file_path_str: &str, all_nodes: &mut HashSet<String>, all_edges: &mut Vec<DependencyEdge>) {
    let source_dir = Path::new(file_path_str).parent().unwrap_or_else(|| Path::new(""));
    for imp in imports {
        if is_noise_import(imp) { continue; }
        let imp = normalize_import(imp);
        if imp.is_empty() { continue; }
        let target = if imp.starts_with("./") || imp.starts_with("../") {
            let resolved = source_dir.join(&imp);
            let resolved_str = resolved.to_string_lossy().replace('\\', "/");
            let mut parts = Vec::new();
            for part in resolved_str.split('/') { match part { ".." if !parts.is_empty() => { parts.pop(); } "." | "" => {} _ => parts.push(part) } }
            if parts.is_empty() { continue; }
            parser::extract_package_name(Path::new(&parts.join("/")))
        } else if imp.contains('/') { parser::extract_package_name(Path::new(&imp)) } else { imp };
        if target.is_empty() || target == source_pkg { continue; }
        all_nodes.insert(target.clone());
        all_edges.push(DependencyEdge { from_module: source_pkg.to_string(), to_module: target, file_path: file_path_str.to_string(), line: 0, weight: 1 });
    }
}

pub struct ScanResult { pub commits_scanned: usize, pub graphs_created: usize, pub drifts_calculated: usize }
...
pub fn run_scan(path: &Path, db: &Database, max_commits: usize) -> Result<ScanResult> {
    ...
    if let Ok(Some(_)) = db.get_graph_snapshot(&head_hash) {
        ...
        return Ok(ScanResult { commits_scanned: 0, graphs_created: 0, drifts_calculated: 0 });
    }
    ...
    let mut commits_raw = if let Some(ref last_hash) = last_commit {
        ...
        info!("Incremental scan: {} existing snapshots, {} new commits", existing_count, new_commits.len());
        new_commits
    } else {
    ...
    for chunk in commit_hashes.chunks(200) {
        ...
        let chunk_results: Vec<Result<(CommitInfo, Vec<DependencyEdge>, HashSet<String>)>> = chunk.par_iter().map(|hash: &String| {
            ...
            let commit_info = CommitInfo { hash: hash.clone(), author_name, author_email, message: decoded.message.to_string(), timestamp, tree_id: decoded.tree().to_string() };

            let tree_oid = commit_obj.tree_id()?.detach();
            let entries = git_scanner::walk_tree_entries_cached(&repo, tree_oid, &ctx.subtree_cache)?;
            ...
            for (file_path, blob_oid) in entries {
                ...
                if let Some(cached) = ctx.blob_import_cache.get(&oid_key) {
                    ...
                    if let Ok(content) = std::str::from_utf8(&blob.data) {
                        if let Some(lang) = parser::detect_language(&file_path.to_string_lossy()) {
                            parse_jobs.push((source_pkg, oid_key, content.to_string(), file_path.clone(), lang));
                        }
                    }
                }
            }
        ...
        for res in chunk_results {
            ...
            for edge in all_edges {
                edge_weight_map.entry((edge.from_module.clone(), edge.to_module.clone()))
                    .and_modify(|e| e.weight += 1).or_insert(edge);
            }
            ...
            for edge in &merged_edges {
                if !source_pkgs.contains(&edge.to_module) {
                    ext_importers.entry(edge.to_module.clone()).or_default().insert(edge.from_module.clone());
                }
            }
            ...
            let drift = scoring::calculate_drift(&graph, prev_graph.as_ref(), &nodes_vec, &scoring::edges_to_pairs(&final_edges), commit_info.timestamp);
            
            db.insert_graph_snapshot(&GraphSnapshot {
                commit_hash: commit_info.hash, nodes: nodes_vec, edges: final_edges,
                node_count: graph.node_count(), edge_count: graph.edge_count(),
                timestamp: commit_info.timestamp, drift: Some(drift),
            })?;
        ...
        info!("[{}/{}] {:.1}s — Batch complete, cache: {}", graphs_created, total_commits, elapsed, ctx.blob_import_cache.len());
    }
